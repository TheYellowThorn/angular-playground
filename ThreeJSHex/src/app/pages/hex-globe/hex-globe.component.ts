...
export class HexGlobeComponent implements OnInit {
  ...
  pieGeometry(height: number, position: Vector2): BufferGeometry {
    ...
    let startingGeo: CylinderGeometry = new CylinderGeometry(
      1,
      1,
      height,
      3,
  ...
  pie(height: number, position: Vector2) {
    ...
    if (height > this.STONE_HEIGHT) {
      this.stoneGeo = BufferGeometryUtils.mergeBufferGeometries([
        geo,
        this.stoneGeo,
      ]);
      ...
      if (Math.random() > 0.8) {
        this.stoneGeo = BufferGeometryUtils.mergeBufferGeometries([
          this.stoneGeo,
          this.stone(height, position),
        ]);
      }
    } else if (height > this.DIRT_HEIGHT) {
      this.dirtGeo = BufferGeometryUtils.mergeBufferGeometries([
        geo,
        this.dirtGeo,
      ]);

      if (Math.random() > 0.8) {
        this.grassGeo = BufferGeometryUtils.mergeBufferGeometries([
          this.grassGeo,
          this.tree(height, position),
        ]);
      }
      ...
      this.sandGeo = BufferGeometryUtils.mergeBufferGeometries([
        geo,
        this.sandGeo,
      ]);
      ...
      this.dirt2Geo = BufferGeometryUtils.mergeBufferGeometries([
        geo,
        this.dirt2Geo,
      ]);
    }
  ...
  stone(height: number, position: Vector2) {
    ...
    const geo: BufferGeometry = new SphereGeometry(
      Math.random() * 0.3 + 0.1,
      7,
      7
    );
  ...
  clouds() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const cloudGeo = BufferGeometryUtils.mergeBufferGeometries([
        puff1,
        puff2,
        puff3,
      ]);
